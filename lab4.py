from peewee import (
    ...
    ForeignKeyField, SqliteDatabase, TextField, OperationalError
)
...
# Функция для преобразования модели в словарь с обработкой datetime и ForeignKey
...
class ChangeLogController:
    ...
    def restore_from_log(self, entity_name, entity_id, log_id, user_id):
        require_authorization(user_id)
        # Формируем разрешение на восстановление сущности
        ...
        try:
            with db.atomic():
                if entity_name == 'User':
                    ...
                    entity = Role.get_by_id(entity_id)
                elif entity_name == 'Permission':
                    entity = Permission.get_by_id(entity_id)
                else:
                    return jsonify({"error": "Неизвестная сущность"}), 400

                # Применяем изменения
                before_state = json.loads(changelog.before)
                for key, value in before_state.items():
                    setattr(entity, key, value)
                entity.created_by = user_id  # Обновляем поле created_by для логирования
                entity.save()
...
def initialize_database():
    db.connect()
    db.create_tables([Role, Permission, User, UserRole, RolePermission, ChangeLog])
...
# Маршруты для работы с логами изменений
# (Добавлены выше, не дублируются здесь)
